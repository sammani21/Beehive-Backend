...
exports.getBeekeeper = tryCatch(async (req, res) => {
  ...
  if (!beekeeper) {
    const errorResponse = {
      statusCode: 404,
      msg: `${req.params.id} beekeeper not found!`,
    };
...
exports.updateBeekeeper = tryCatch(async (req, res) => {
  ...
  try {
    ...
    await sendEmail(
      beekeeper.email || updateFields.email,
    ...
    return res
      ...
      .json({
        status: true,
        message: "Beekeeper updated successfully",
        data: beekeeper,
      });
  } catch (err) {
...
exports.toggleBeekeeperStatus = tryCatch(async (req, res) => {
  ...
  await sendEmail(
    ...
    `Your account status has been changed to ${
      beekeeper.isActive ? "Active" : "Inactive"
    }.
  ...
  res
    ...
    .json({
      status: true,
      message: `Beekeeper ${
        beekeeper.isActive ? "activated" : "deactivated"
      } successfully`,
      data: updatedBeekeeper,
    });
});
